...
// keeps checking for any ok messages
...
// keeps checking for any coordinator messages
async function wait_for_coordinator_messages() {
  // wait up to 100 seconds for a COORDINATOR
  const totalWait = 100000;
  ...
  try{
      ...
      if(higherPeers.length === 0){
        ...
        if(ok_messages){
          ...
          // election_state was set to  "WAIT_COORDINATOR"
          const coordinator_messages = await wait_for_coordinator_messages();
          if(!coordinator_messages){
            console.log("❌ No COORDINATOR after 100s → Restart the election...");
